...
if (count _tasks > 0) then {
    ...
    private _timeLastProcess = _currTask getvariable "timeLastProcess";
    if (time - _timeLastProcess > 1) then {
        ...
        if (!_hasExecuted) then {
            ...
            if (_isEndState) then {
                private _callback = _currTask getvariable "callback";
                private _callbackArgs = _currTask getvariable "callbackArgs";
                ([_currTask] + _callbackArgs) call _callback;
        ...
        _currTask setvariable ["timeLastProcess", time];
    };
};